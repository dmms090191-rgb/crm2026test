/*
  ...
  ## Tables Created
  1. `registrations`         — Incoming registration requests from new users
  2. `import_history`        — Tracks CSV import operations (file, counts, mode)
  3. `leads`                 — Core lead/prospect data, linked to imports and vendors
  4. `vendors`               — Salesperson accounts with Supabase Auth link
  5. `vendor_comments`       — Admin comments on vendor profiles
  6. `vendor_admin_messages` — Messaging thread between vendors and admin
  7. `rdv_proposals`         — Appointment proposals submitted by vendors
  8. `client_messages`       — Messaging thread between clients and admin/vendors
  9. `statuts`               — Custom CRM lead statuses with colors
  10. `crm_documentation`    — Rich text documentation tabs (persisted content)
  11. `crm_notes`            — Internal dated notes
  12. `crm_ideas`            — CRM improvement ideas with kanban status
  13. `crm_context_cards`    — Context cards for AI/ChatGPT integration
  14. `sidebar_order`        — Persisted sidebar navigation order per group/item
  ...
  - `update_crm_documentation_updated_at()` — Trigger: keeps updated_at current
  - `update_crm_context_cards_updated_at()` — Trigger: keeps updated_at current
...
CREATE TABLE IF NOT EXISTS import_history (
  ...
  import_mode      text        DEFAULT 'ignore',
  source_file      text,
  imported_by      uuid
);
