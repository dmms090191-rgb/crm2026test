...
export default function CSAAdminDetailCommentsTab({ adminId }: Props) {
  const tokens = useThemeTokens();
  const [comments, setComments] = useState<CSAComment[]>([]);
  ...
  async function addComment() {
    ...
    await supabase.from('csa_admin_comments').insert({
      admin_id: adminId,
      author_id: user.id,
  ...
  async function deleteSelected() {
    ...
    await supabase.from('csa_admin_comments').delete().in('id', Array.from(selected));
    setSelected(new Set());
    await fetchComments();
  }
  ...
  async function deleteAll() {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) return;
    await supabase.from('csa_admin_comments').delete().eq('admin_id', adminId).eq('author_id', user.id);
    setSelected(new Set());
    await fetchComments();
  }
