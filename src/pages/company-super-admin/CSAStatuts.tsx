...
import { Statut, PRESET_COLORS, MAX_FAVORITES, colorWithAlpha } from '../admin/views/statuts/colorUtils';
import StatutsList from '../admin/views/statuts/StatutsList';

const CSA_FAVORITES_KEY = 'csa_statuts_favorite_colors';
const CSA_PROTECTED_STATUTS = ['Nouveau'];

export default function CSAStatuts({ groupCompanyId }: { groupCompanyId: string }) {
  ...
  const [favorites, setFavorites] = useState<string[]>(() => {
    try { return JSON.parse(localStorage.getItem(CSA_FAVORITES_KEY) ?? '[]'); } catch { return []; }
  });
  const colorInputRef = useRef<HTMLInputElement>(null);

  const load = useCallback(async () => {
    if (!groupCompanyId) { setStatuts([]); setLoading(false); return; }
  ...
  const saveFavorites = (favs: string[]) => {
    setFavorites(favs);
    localStorage.setItem(CSA_FAVORITES_KEY, JSON.stringify(favs));
  };

  const addToFavorites = (color: string) => {
    if (favorites.includes(color)) return;
    saveFavorites([color, ...favorites].slice(0, MAX_FAVORITES));
  };
  ...
  const handleCreate = async () => {
    ...
    if (duplicate) { setError('Un statut avec ce nom existe deja.'); return; }
    setSaving(true);
    const { error: err } = await supabase
      .from('csa_statuts')
      .insert({ nom: nom.trim(), couleur, group_company_id: groupCompanyId });
    if (err) {
      setError(err.code === '23505' ? 'Un statut avec ce nom existe deja.' : 'Erreur lors de la creation.');
      setSaving(false);
      return;
    }
    setNom('');
  ...
  const handleDelete = async (id: string) => {
    const target = statuts.find(s => s.id === id);
    if (target && CSA_PROTECTED_STATUTS.includes(target.nom)) return;
    await supabase.from('csa_statuts').delete().eq('id', id).eq('group_company_id', groupCompanyId);
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      <div>
        ...
        <p className="text-xs mt-0.5" style={{ color: tokens.label.hint }}>Gerez les statuts utilises pour classer vos Societes</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          <h3 className="text-sm font-bold" style={{ color: tokens.text.primary }}>Creer un statut</h3>
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              placeholder="Ex: Nouveau, A contacter..."
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between">
              ...
              >
                ...
                <div className="w-4 h-4 rounded-md" style={{ background: couleur }} />
              </div>
              <button
          ...
          <div
            ...
            style={{ background: tokens.surface.secondary, border: `1px solid ${tokens.surface.borderLight}` }}
          >
            <p className="text-[9px] font-bold tracking-[0.15em] uppercase mb-3" style={{ color: tokens.label.hint }}>Apercu</p>
          ...
          >
            ...
            {saving ? 'Creation...' : 'Creer le statut'}
          </button>
        </div>

        <StatutsList statuts={statuts} loading={loading} onDelete={handleDelete} tokens={tokens} protectedNames={CSA_PROTECTED_STATUTS} />
      </div>
    </div>
  );
}
