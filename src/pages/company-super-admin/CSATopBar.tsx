...
import MessageBubblePopover, { type BubbleEntry } from '../../components/notifications/MessageBubblePopover';
...
interface Props {
  ...
  unreadAdminMsgEntries?: BubbleEntry[];
  onAdminMsgEntryClick?: (id: string) => void;
}

const VIEW_TITLES: Record<CSAView, string> = {
  overview: 'Dashboard',
  admins: 'Gestion des sociétés',
  statuts: 'Statuts',
  info: 'Accès & sécurité',
  'chat-admin': 'Chat Sociétés',
  'chat-rois-admin': 'Chat Talvex Administrateur',
    ...
    <>
      ...
      >
        <div className="flex items-center gap-1.5 sm:gap-2 min-w-0">
          ...
          <span className="text-xs hidden md:inline" style={{ color: tTopbar.breadcrumbPrefix || tTopbar.breadcrumbSep }}>GROUPE</span>
        ...
        <div className="flex items-center gap-1 sm:gap-2 min-w-0">
          ...
          <div className="relative flex-shrink-0" ref={msgDropRef}>
            <MessagesBubbleTrigger
              count={unreadAdminMsgCount}
              onClick={() => setMsgDropOpen(prev => !prev)}
              mutedColor={tTopbar.breadcrumbPrefix}
            />
            {msgDropOpen && (
              <MessageBubblePopover
                entries={unreadAdminMsgEntries}
                onEntryClick={id => { onAdminMsgEntryClick?.(id); setMsgDropOpen(false); }}
                tokens={t}
              />
            )}
          </div>
