import { FileText, CheckCircle, Eye, X, Users, Hash } from 'lucide-react';
...
export default function PreviewPhase({
  file, counts, allColumns, mapping, processedRows,
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div key={s.label} className="rounded-2xl p-4" style={{ background: tokens.card.bg, border: tokens.card.border }}>
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs" style={{ color: tokens.text.tertiary }}>{s.label}</span>
              <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: s.bg, color: s.color, border: `1px solid ${s.border}` }}>{s.icon}</div>
            </div>
      ...
      {Object.values(mapping).some(Boolean) && (
        <div className="flex items-center gap-3 flex-wrap px-4 py-3 rounded-xl" style={{ background: 'rgba(34,211,238,0.05)', border: tokens.accent.border }}>
          <span className="text-[10px] font-bold tracking-widest uppercase" style={{ color: tokens.accent.text }}>Colonnes detectees</span>
      ...
      <div className="rounded-2xl overflow-hidden" style={{ background: tokens.card.bg, border: tokens.card.border }}>
        <div className="flex items-center gap-2 px-5 py-3.5" style={{ borderBottom: tokens.table.headerBorder }}>
          <Eye className="w-4 h-4" style={{ color: tokens.accent.text }} />
          <h3 className="text-sm font-semibold" style={{ color: tokens.text.primary }}>Previsualisation</h3>
          <span className="text-xs ml-auto" style={{ color: tokens.text.quaternary }}>Survolez le badge de statut pour voir le detail</span>
        </div>
        <PreviewTable rows={processedRows} allColumns={allColumns} mappedColumns={mapping} />
      </div>

      <div className="flex items-center justify-between">
        <button onClick={onReset} className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-semibold transition-all" style={{ color: tokens.text.tertiary, background: tokens.surface.tertiary, border: tokens.card.border }}>
