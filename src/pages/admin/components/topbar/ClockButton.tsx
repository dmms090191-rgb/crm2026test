...
export default function ClockButton({ tzLabel, tzCode, clock, onClick }: ClockButtonProps) {
  return (
    ...
    >
      ...
      <span className="text-xs font-medium hidden lg:block" style={{ color: '#64748b' }}>{tzLabel}</span>
      {tzCode && <span className="text-xs font-medium hidden sm:block lg:hidden" style={{ color: '#64748b' }}>{tzCode}</span>}
