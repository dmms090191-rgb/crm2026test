...
interface Props {
  open: boolean;
  item: TestTranslation | null;
  categories: TestCategoryItem[];
  onMove: (itemId: string, targetCategoryId: string) => void;
  onClose: () => void;
}

export default function SATestMoveModal({ open, item, categories, onMove, onClose }: Props) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      >
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-base font-bold text-slate-100">Deplacer la traduction</h2>
        ...
        <div className="mb-4 px-3 py-2 rounded-lg" style={{ background: t.modal.fieldBg, border: `1px solid ${t.modal.fieldBorder}` }}>
          <p className="text-[11px] text-slate-400 mb-1">Traduction :</p>
          <p className="text-xs font-semibold text-slate-200 truncate">{item.testName}</p>
          <p className="text-[10px] text-slate-500 mt-1">
            Actuellement dans : <span className="text-amber-400">{currentCat?.name || 'Autres'}</span>
          </p>
        </div>
