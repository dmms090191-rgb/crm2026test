...
export default function SASuperAdminCreateModal({ onClose, onCreated, tokens: t }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/create-company-super-admin`,
        {
          method: 'POST',
          headers: {
      ...
      <div className="w-full max-w-md rounded-2xl overflow-hidden" style={{ background: t.modal.bg, border: `1px solid ${t.modal.border}`, boxShadow: t.modal.shadow }}>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="superadmin@entreprise.com" className="w-full px-3 py-2 rounded-lg text-sm outline-none transition-all" style={{ background: t.modal.fieldBg, border: `1px solid ${t.modal.fieldBorder}`, color: t.modal.fieldValue }} />
          </div>
          <div>
