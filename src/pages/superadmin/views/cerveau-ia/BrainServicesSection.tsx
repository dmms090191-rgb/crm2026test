...
export default function BrainServicesSection({ brain, onChange, tokens: t }: Props) {
              ...
              <div className="flex items-start justify-between gap-2">
                <div className="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-2">
                  ...
                  <input
                    ...
                    className="px-2.5 py-1.5 rounded-lg text-xs outline-none sm:col-span-1"
                    style={{ background: t.input.bg, border: `1px solid ${t.input.border}`, color: t.input.text }}
                  />
                  <input
                    value={svc.price}
                    onChange={e => updateService(idx, 'price', e.target.value)}
                    placeholder="Prix (ex: 50EUR)"
                    className="px-2.5 py-1.5 rounded-lg text-xs outline-none"
                    style={{ background: t.input.bg, border: `1px solid ${t.input.border}`, color: t.input.text }}
                  />
                </div>
