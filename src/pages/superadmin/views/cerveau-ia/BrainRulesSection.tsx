...
export default function BrainRulesSection({ brain, onChange, tokens: t }: Props) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      <div className="rounded-2xl p-5" style={{ background: t.card.bg, border: `1px solid ${t.card.border}` }}>
        <div className="flex items-center gap-2.5 mb-4">
          <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ background: t.accent.bg, border: `1px solid ${t.accent.border}` }}>
            <CalendarCheck className="w-4 h-4" style={{ color: t.accent.text }} />
          </div>
          ...
          {([
            ...
            <div key={field} className="flex items-center justify-between gap-3">
              ...
              <input
                ...
                min={0}
                value={val}
                onChange={e => updateApt(field, parseInt(e.target.value) || 0)}
      ...
      <div className="rounded-2xl p-5" style={{ background: t.card.bg, border: `1px solid ${t.card.border}` }}>
        <div className="flex items-center gap-2.5 mb-4">
          <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ background: t.accent.bg, border: `1px solid ${t.accent.border}` }}>
            <Database className="w-4 h-4" style={{ color: t.accent.text }} />
          </div>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between gap-3">
            ...
            <input
              value={crm.default_status}
              onChange={e => updateCrm('default_status', e.target.value)}
            ...
            <div key={field} className="flex items-center justify-between gap-3">
              ...
              <label className="relative inline-flex items-center cursor-pointer">
                <input
                  type="checkbox"
                  checked={val}
                  onChange={e => updateCrm(field, e.target.checked)}
                  className="sr-only peer"
                />
                <div
                  className="w-9 h-5 rounded-full transition-colors peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all"
                  style={{ background: val ? t.accent.text : t.surface.border }}
                />
              </label>
            </div>
          ))}
