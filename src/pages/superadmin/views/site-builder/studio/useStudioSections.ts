...
export default function useStudioSections(
    ...
    try {
      const [{ data, error }, { data: pageRow }] = await Promise.all([
        ...
        supabase
          ...
          .select('draft_canvas_bg_desktop, draft_canvas_bg_mobile, draft_canvas_gradient_desktop, draft_canvas_gradient_mobile, draft_page_size_desktop, draft_page_size_mobile, draft_canvas_bg_mode_desktop, draft_canvas_bg_mode_mobile')
          .eq('id', homePageId)
          .maybeSingle(),
      ]);
  ...
  const publish = async (): Promise<boolean> => {
    ...
    try { await persistPublish(homePageId, dbSections, dbCanvasBg, dbCanvasGradient, dbPageHeight, dbBgMode); setIsPublished(true); await loadSections(); return true; }
