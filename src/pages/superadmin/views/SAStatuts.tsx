...
import { Statut, PRESET_COLORS, MAX_FAVORITES, FAVORITES_KEY, colorWithAlpha } from '../../admin/views/statuts/colorUtils';
import StatutsList from '../../admin/views/statuts/StatutsList';

const SA_FAVORITES_KEY = 'sa_statuts_favorite_colors';
const SA_PROTECTED_STATUTS = ['Nouveau'];

export default function SAStatuts() {
  ...
  const [favorites, setFavorites] = useState<string[]>(() => {
    try { return JSON.parse(localStorage.getItem(SA_FAVORITES_KEY) ?? '[]'); } catch { return []; }
  });
  const colorInputRef = useRef<HTMLInputElement>(null);

  const load = useCallback(async () => {
    const { data } = await supabase.from('sa_statuts').select('*').order('created_at', { ascending: true });
  ...
  const saveFavorites = (favs: string[]) => {
    setFavorites(favs);
    localStorage.setItem(SA_FAVORITES_KEY, JSON.stringify(favs));
  };
  ...
  const handleCreate = async () => {
    ...
    setSaving(true);
    const { error: err } = await supabase.from('sa_statuts').insert({ nom: nom.trim(), couleur });
    if (err) { setError('Erreur lors de la creation.'); setSaving(false); return; }
    setNom('');
    setSaving(false);
    load();
  };
  ...
  const handleDelete = async (id: string) => {
    ...
    if (target && SA_PROTECTED_STATUTS.includes(target.nom)) return;
    await supabase.from('sa_statuts').delete().eq('id', id);
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      <div>
        ...
        <p className="text-xs mt-0.5" style={{ color: tokens.label.hint }}>Gerez les statuts pour CRM Societe / Societes prospects</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <StatutsList statuts={statuts} loading={loading} onDelete={handleDelete} tokens={tokens} protectedNames={SA_PROTECTED_STATUTS} />
      </div>
    </div>
  );
}
