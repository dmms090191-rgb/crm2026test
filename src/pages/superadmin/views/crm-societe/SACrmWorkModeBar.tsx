...
export default function SACrmWorkModeBar({
  ...
  return (
    <div className="px-3 md:px-5 py-2.5 md:py-3 space-y-2 md:space-y-0" style={{ borderBottom: `1px solid ${t.surface.border}` }}>
      {/* Desktop */}
      <div className="hidden md:flex items-center gap-3">
        ...
        {workModeEnabled && (
          <div className="flex items-center gap-2 ml-1 relative">
            ...
            {showToast && (
              <div className="absolute -bottom-8 left-0 z-50 rounded-md px-2.5 py-1 shadow-lg text-[10px] font-medium whitespace-nowrap" style={{ background: t.card.bg, border: `1px solid ${t.surface.border}`, color: '#f97316' }}>
      ...
      <div className="flex md:hidden flex-col gap-2">
        <div className="flex items-center gap-2">
          ...
          {selectMode && (
            <label className="flex items-center gap-1.5 cursor-pointer select-none px-2 py-2 rounded-lg" style={{ background: t.surface.hover, border: `1px solid ${t.surface.borderLight}` }}>
              <div style={checkboxStyle(allChecked, t.surface.border)} onClick={toggleAll}>
                {(allChecked || someChecked) && <Check className="w-2.5 h-2.5 text-white" />}
              </div>
              <span className="text-[11px] font-medium" style={{ color: t.text.secondary }}>Tout</span>
        ...
        {workModeEnabled && (
          <div className="flex items-center gap-2 relative">
            <div className="flex items-center rounded-lg overflow-hidden flex-shrink-0" style={{ border: '1px solid rgba(249,115,22,0.2)' }}>
              <button onClick={onUndo} disabled={!canUndo} className="w-9 h-9 flex items-center justify-center transition-colors disabled:opacity-25" style={{ color: '#f97316' }}>
                <Undo2 className="w-4 h-4" />
              </button>
              <span className="text-[12px] font-bold tabular-nums px-2.5 h-9 flex items-center border-x" style={{ color: '#f97316', background: 'rgba(249,115,22,0.04)', borderColor: 'rgba(249,115,22,0.15)' }}>
                {historyPosition}/{historyLength}
              </span>
              <button onClick={onRedo} disabled={!canRedo} className="w-9 h-9 flex items-center justify-center transition-colors disabled:opacity-25" style={{ color: '#f97316' }}>
                <Redo2 className="w-4 h-4" />
              </button>
            </div>

            <button onClick={onLocate} disabled={!canLocate} className="w-9 h-9 rounded-lg flex items-center justify-center transition-all disabled:opacity-25 flex-shrink-0" style={{ background: 'rgba(249,115,22,0.06)', border: '1px solid rgba(249,115,22,0.2)', color: '#f97316' }}>
              <LocateFixed className="w-4 h-4" />
            </button>

            {historyLength > 0 && (
              <button onClick={handleReset} className="flex items-center gap-1.5 h-9 px-3 rounded-lg transition-all active:scale-95 ml-auto flex-shrink-0" style={{ background: 'rgba(249,115,22,0.08)', border: '1px solid rgba(249,115,22,0.2)', color: '#f97316' }}>
            ...
            {showToast && (
              <div className="absolute -bottom-7 left-0 z-50 rounded-md px-2.5 py-1 shadow-lg text-[10px] font-medium whitespace-nowrap" style={{ background: t.card.bg, border: `1px solid ${t.surface.border}`, color: '#f97316' }}>
                Historique reinitialise
              </div>
            )}
          </div>
