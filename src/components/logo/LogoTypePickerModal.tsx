...
export default function LogoTypePickerModal({ selected, onConfirm, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-3 sm:p-6"
      ...
      <div className="w-full max-w-lg rounded-2xl overflow-hidden flex flex-col" style={{
        ...
        <div style={{ background: 'linear-gradient(135deg, #f59e0b, #d97706, #b45309)', padding: '2px 0 0 0' }}>
          <div className="flex items-center justify-between px-5 py-4" style={{ background: t.surface.primary }}>
            <div className="flex items-center gap-3.5">
              ...
              <div>
                <h3 className="text-[14px] font-extrabold" style={{ color: t.text.primary }}>Types de logo</h3>
                <div className="flex items-center gap-2 mt-0.5">
                  <span className="text-[11px] font-medium" style={{ color: t.text.tertiary }}>
                    Selectionnez jusqu'a {MAX_SELECTED_PRESETS} types
                  </span>
                  <span className="inline-flex items-center justify-center px-2 py-0.5 rounded-md text-[10px] font-black tabular-nums"
                    style={{
        ...
        <div className="flex-1 overflow-y-auto p-5">
            ...
            {PRESETS.map(p => {
              ...
              return (
                <button key={p.id} type="button" onClick={() => !dis && toggle(p.id)}
                  className="group relative flex items-start gap-3.5 p-4 rounded-xl text-left transition-all"
                  style={{
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-[12px] font-extrabold" style={{ color: sel ? '#92400e' : t.text.primary }}>{p.label}</span>
                    </div>
                    <p className="text-[10.5px] leading-relaxed font-medium" style={{ color: t.text.tertiary }}>{p.desc}</p>
                  </div>
                </button>
              );
            })}
          ...
          {draft.length === MAX_SELECTED_PRESETS && (
            <div className="mt-4 flex items-start gap-3 rounded-xl px-4 py-3" style={{
              ...
              <Layers className="w-4 h-4 flex-shrink-0 mt-0.5" style={{ color: '#0ea5e9' }} />
              <p className="text-[10.5px] leading-relaxed font-medium" style={{ color: t.text.secondary }}>
        ...
        {/* Footer */}
        <div className="px-4 sm:px-5 py-3 sm:py-4 flex items-center justify-end gap-2 sm:gap-3" style={{ borderTop: `1px solid ${t.surface.border}` }}>
          <button onClick={onClose}
            className="px-4 sm:px-5 py-2.5 sm:py-2.5 rounded-xl text-[12px] font-bold transition-all active:scale-95"
          ...
          <button onClick={() => { if (draft.length > 0) onConfirm(draft); }} disabled={draft.length === 0}
            className="relative overflow-hidden flex-1 sm:flex-none px-5 sm:px-6 py-2.5 rounded-xl text-[12px] font-extrabold transition-all disabled:opacity-35 active:scale-[0.97]"
            style={{
