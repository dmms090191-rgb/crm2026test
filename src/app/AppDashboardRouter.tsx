...
function RouterInner({
  ...
  const canHideTabs = role === 'super_admin' || visu.originalRole === 'super_admin';
  ...
  if (role === 'super_admin' && impersonatedCompanySuperAdmin) {
    return <CompanySuperAdminDashboard impersonated={impersonatedCompanySuperAdmin} onBack={backFromCSA} isImpersonation visuBadgeLabel={badge} backLabel={back} canHideTabs={canHideTabs} />;
  }
    ...
    return (
      <DemoSessionProvider saUserId={saUserId ?? undefined} saDisplayName={saDisplayName}>
        <AppShell panelRole="client" useCompanyProvider companyId={impersonatedAdmin.company_id} effectiveUserId={impersonatedClient.id}>
          <ClientDashboard onLogout={onLogout} impersonatedClient={impersonatedClient} onBackToAdmin={backFromClient} backLabel={back || 'Retour vendeur'} isSAViewing visuBadgeLabel={badge} canHideTabs={canHideTabs}
    ...
    return (
      <DemoSessionProvider saUserId={saUserId ?? undefined} saDisplayName={saDisplayName}>
        <AppShell panelRole="vendor" useCompanyProvider companyId={impersonatedAdmin.company_id} effectiveUserId={impersonatedVendor.auth_user_id ?? impersonatedVendor.id}>
          <VendorDashboard onLogout={onLogout} impersonatedVendor={impersonatedVendor} onBackToAdmin={backFromVendor} onConnectAsClient={connectAsClientFromVendor} isSAViewing visuBadgeLabel={badge} backLabel={back} canHideTabs={canHideTabs}
  ...
  if (role === 'super_admin' && impersonatedAdmin && impersonatedClient) {
    return (
      <DemoSessionProvider saUserId={saUserId ?? undefined} saDisplayName={saDisplayName}>
        <AppShell panelRole="client" useCompanyProvider companyId={impersonatedAdmin.company_id} effectiveUserId={impersonatedClient.id}>
          <ClientDashboard onLogout={onLogout} impersonatedClient={impersonatedClient} onBackToAdmin={backFromClient} backLabel={back || 'Retour admin'} isSAViewing visuBadgeLabel={badge} canHideTabs={canHideTabs}
            hideTabsTargetName={`${impersonatedClient.prenom} ${impersonatedClient.nom}`.trim() || impersonatedClient.email}
            hideTabsTargetUserId={impersonatedClient.id} />
        </AppShell>
      </DemoSessionProvider>
  ...
  if (role === 'super_admin' && impersonatedAdmin) {
    const adminTargetName = `${impersonatedAdmin.first_name} ${impersonatedAdmin.last_name}`.trim() || impersonatedAdmin.email;
    return (
      <DemoSessionProvider saUserId={saUserId ?? undefined} saDisplayName={saDisplayName}>
        <AppShell panelRole="admin" useCompanyProvider companyId={impersonatedAdmin.company_id}>
          <AdminDashboard onLogout={onLogout} onConnectAsVendor={connectAsVendor} onConnectAsClient={connectAsClientFromAdmin}
            impersonatedAdmin={impersonatedAdmin} onBackToSuperAdmin={backFromAdmin} isSAViewing visuBadgeLabel={badge} backLabel={back} canHideTabs={canHideTabs}
            hideTabsTargetName={adminTargetName} hideTabsTargetUserId={impersonatedAdmin.id} />
        </AppShell>
      </DemoSessionProvider>
    );
  }
